...
- Код: `Lab5/main.cpp` (интерактивное приложение: команды читаются из stdin, ответы печатаются в stdout).
- Стратегии: `ordered` (последовательно построчно) и `custom` (случайные неповторяющиеся выстрелы).
- Сборка: в репозитории нет CMake для этой лабы — можно собрать любым C++ компилятором (например, из `Lab5/`: `g++ -std=c++20 main.cpp -O2 -o labwork5`).
