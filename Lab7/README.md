...
- Контейнер: `Lab7/lib/unrolled_list.h` (шаблон `unrolled_list<T, NodeMaxSize, Allocator>`).
- Пример: `Lab7/bin/main.cpp` (исполняемый файл `unrolled_list`).
- Тесты: `Lab7/tests/*_ut.cpp` (тестовый бинарь `unrolled-list-lib-tests`, подтягивает GoogleTest через CMake FetchContent).
- Сборка/тесты: из `Lab7/` — `cmake -S . -B build`, `cmake --build build`, `ctest --test-dir build -V`.
