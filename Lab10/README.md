...
- Исполняемый файл `itmoscript_interpreter`: `Lab10/bin/main.cpp`.
- Код интерпретатора: `Lab10/bin/lexer.cpp`, `Lab10/bin/parser.cpp`, `Lab10/bin/interpreter.cpp` (заголовки — `Lab10/lib/*`).
- Тесты: `Lab10/tests/*_test.cpp` (тестовый бинарь `itmoscript_tests`, подтягивает GoogleTest через CMake FetchContent).
- Сборка/тесты: из `Lab10/` — `cmake -S . -B build`, `cmake --build build`, `ctest --test-dir build -V`.
