...
- Режим: по дате и списку городов из `cities.txt` строится оптимальный маршрут, проходящий через все города (старт — первый, финиш — последний) на основе данных API Яндекс Расписаний.
- Запуск: `labwork6 <date> <N>`, где `<date>` — `YYYY-MM-DD`, `<N>` — количество городов в `cities.txt`.
- Файлы: нужен `cities.txt` (N строк/слов с названиями городов) и папка `cache/` для кэша (`cache/stations.json`, `cache/ways.json`).
- Сборка: см. `Lab6/CMakeLists.txt` (используется vcpkg toolchain и пакеты `cpr`, `nlohmann_json`).
