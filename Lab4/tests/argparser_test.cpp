...
TEST(ArgParserTestSuite, DefaultTest2) {
    ArgParser parser("My Parser");
    parser.AddStringArgument({"param1", "param2"}).Default("value1");
    
    ASSERT_TRUE(parser.Parse(SplitString("app")));
    ASSERT_EQ(parser.GetStringValue("param2"), "value1");
}
...
TEST(ArgParserTestSuite, MultiValueTest) {
    ...
    parser.AddStringArgument('p', {"param1"}).MultiValue().StoreValues(&int_values);
...
TEST(ArgParserTestSuite, MinCountMultiValueTest) {
    ...
    parser.AddStringArgument('p', {"param1"}).MultiValue(MinArgsCount).StoreValues(&int_values);
