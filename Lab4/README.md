...
- Код парсера: `Lab4/lib/ArgParser.h`, `Lab4/lib/ArgParser.cpp`.
- Пример: `Lab4/bin/main.cpp`.
- Тесты: `Lab4/tests/argparser_test.cpp` (тестовый бинарь `argparser_tests`, подтягивает GoogleTest через CMake FetchContent).
- Сборка/тесты: из `Lab4/` — `cmake -S . -B build`, `cmake --build build`, `ctest --test-dir build -V`.
