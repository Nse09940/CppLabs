...
- Код: `Lab8/lib/processing.h`.
- Пример: `Lab8/bin/main.cpp`
- Тесты: `Lab8/tests/*_ut.cpp` (тестовый бинарь `processing-lib-tests`, подтягивает GoogleTest через CMake FetchContent).
- Сборка/тесты: из `Lab8/` — `cmake -S . -B build`, `cmake --build build`, `ctest --test-dir build -V`.
