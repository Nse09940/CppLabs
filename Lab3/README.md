...
- Код: `Lab3/bin/main.cpp` (консольное приложение).
- Аргументы: в текущей реализации ожидается формат с отдельным `=`: `-i = <input.tsv>`, `-o = <output.bmp>`, `-m = <max_iter>`, `-f = <freq>`.
- Вывод: сохраняет итоговое состояние в BMP по пути из `-o`.
